...
public class gui_ds extends JPanel implements ActionListener
{
	/*--------------定义组件-------------------*/
	/*标签*/
	JLabel jl=new JLabel("批量代收输入界面DS：");
	...
	JLabel jl_filepath=new JLabel("代收模板文件路径");
	JLabel jl_curcde=new JLabel("交易货币");
	JLabel jl_promocode=new JLabel("收费选项");
	JLabel jl_inaccount=new JLabel("转入账号");
	...
	JTextField jtf_inaccount=new JTextField();
	...
	gui_ds()
	{
		...
		jbr.setString("进度条");
		jbr.setBackground(Color.orange);
	...
	void initialpanel()
	{
        ...
        jl_inaccount.setBounds(30,370,150,30);
        jtf_inaccount.setBounds(100,370,150,30);
	...
	public boolean MyCheck()
	{
		...
		{
			JOptionPane.showMessageDialog(this,"代收模板文件路径","纯情小可爱",JOptionPane.ERROR_MESSAGE);
			b=false;
			jtf_filepath.grabFocus();
		}
		
		if(jtf_inaccount.getText().trim().equals(""))
		{
			JOptionPane.showMessageDialog(this,"转入帐户不能为空","纯情小可爱",JOptionPane.ERROR_MESSAGE);
			b=false;
			jtf_inaccount.grabFocus();
		}
	...
	public class run_JProgressBar extends Thread
	{
		public void run()
		{
			...
			/*产生文本*/
            guids=new formatagnt_ds(gui_orgidt,gui_dealnum,gui_tlr,gui_visanum,gui_ter,gui_trancode,gui_notedate,gui_curcde,gui_promocode,filepath,gui_inaccount,filename,outfilename);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{		
			if(this.MyCheck())
			{
	            ...
	            gui_inaccount=jtf_inaccount.getText().trim();
	            System.out.println("gui转入账号为："+gui_inaccount);
 	   ...
 	   if(e.getSource()==jb_download)
	   {
           ...
           if (returnValue==JFileChooser.APPROVE_OPTION)
           {
           	   ...
           	   System.out.println("魔兽文件产生路径："+jfc_out.getCurrentDirectory().toString());
	...
	{
		gui_ds l=new gui_ds();
		JFrame jf=new JFrame("BPS预处理系统--批量代收ds");
		jf.add(l);
		jf.setVisible(true);
