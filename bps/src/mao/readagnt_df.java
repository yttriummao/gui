...
public class readagnt_df
{
	...
	readagnt_df(String filepath)
	{
		...
		catch(Exception e)
		{
			...
			System.out.println("未找到代付电子模板文件，请检查是否已上传!!");
			System.exit(0);
		}
	}
	...
	*/
	...
	/* 该方法用于获得转出新账号 */
	public String get_out_account_new(String out_account)
	{
		String straccount="";
		
		if(out_account.length()==12||out_account.length()==16)
		{
			straccount=out_account;  //转出账号长度为12位，被认为是新账号
		}
		else
		{
			straccount="";
	...
	/* 该方法用于获得转出旧账号 */
	public String get_out_account_old(String out_account)
	{
		String straccount="";
		
		if(out_account.length()==17||out_account.length()==21)
		{
			straccount=out_account;  //转出账号长度为17位或21位，被认为是旧账号
		}
		else
		{
			straccount="";
	...
	*/
	public String deal_with_body(int c_num,String c_body_orgidt,String c_body_trancode,String c_body_account,String c_body_promocode)
	{
		String restr="";
		
		String tmp="";
		
		System.out.println("########################文件体部分###########################");
		...
		this.body_newoutaccount=format_parameter(get_out_account_new(c_body_account),17,"0","left");
		//System.out.println("转出新账号："+this.body_newoutaccount);
		
		this.body_oldoutaccount=format_parameter(get_out_account_old(c_body_account),25," ","right");
		//System.out.println("转出旧账号："+this.body_oldoutaccount);
		
		this.body_outcard=return_blank(19);    //"转出卡号"，批量代收付中暂无从卡中转出的情况，因此这里填充默认的19个空格
		...
		this.body_newinaccount=format_parameter(get_list_value(num,0),17,"0","left");
		//System.out.println("转入新账号："+this.body_newinaccount);
		
		/*
		this.body_oldinaccount=format_parameter(get_list_value(num,1),25," ","right");
		...
		*/
		...
		if(!tmp.equals(""))
		{
			this.body_oldinaccount=format_parameter("52"+get_list_value(num,1),25," ","right");
		    //System.out.println("转入旧账号："+this.body_oldinaccount);
		}
		else
		{
			this.body_oldinaccount=format_parameter(" ",25," ","right");
		    //System.out.println("转入旧账号："+this.body_oldinaccount);
		}
		
		this.body_incard=format_parameter(get_list_value(num,2),19," ","right");
		...
		this.body_passno=return_blank(30); //"证件号码"，选填部分 全角
		//this.body_passno=return_blank(32); //"证件号码"，选填部分 全角
		//System.out.println("证件号码为："+this.body_passno);
		
		this.body_checkout_type="0";                //"校验种类"
		
		this.body_checkout_direct=return_blank(1);  //"校验方向"
